...
class Face_Register:
    ...
    def GUI_info(self):
        ...
        # Step 4: Halt the program and save the csv automatically
        # HLT button to halt the program
        tk.Button(self.frame_right_info, text='HLT', command=self.halt_program).grid(row=10, column=1, sticky=tk.W)
    ...
    def halt_program(self):
        """Halt the program and close the application."""
        logging.info("Halting the program.")
        self.cap.release()  # Release the video capture
        # Run the features extraction script before closing
